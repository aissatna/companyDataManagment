    ...
    <div class="content">
        ...
        <form class="form_DemmandeRapport" method="post" action="">
            ...
            <div class="wrap_input_DemandeRap m_b_26">
                <label for="contenuSynthese" class="label_input_DemRap">Synthèse</label>
                <textarea class="textArea_DemRap m_l_26" id="contenuSynthese" name="contenuSynthese"  required></textarea>
            </div>
            <div class="wrap_form_btn_DemmandeRap">
                <button type="submit" class="form_btn m_l_26" name="submit">Ajouter</button>
            </div>
        </form>
    </div>
    <?php
    if (isset($_POST['submit'])){
        if (!empty($_POST['contenuSynthese'])){
            // Get entered data and create a legal SQL string that can be used in an SQL statement
            $contenuSynthese = mysqli_real_escape_string($conn,$_POST['contenuSynthese']);
            $sqlQuerUpdate = "UPDATE rapport SET SyntheseRap = '$contenuSynthese'WHERE IdRap = $idRapportChoisi ";
            $resultUpdate = mysqli_query($conn,"$sqlQuerUpdate");
            if($resultUpdate)
                //Success
            {
                echo '<script type="text/javascript">
                            var div_message = document.getElementById(\'js_form_messages\');
                            div_message.innerHTML += \'Votre synthèse est ajoutée  \';
